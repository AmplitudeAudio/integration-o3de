...
#include <Tools/Editor/AmplitudeAudioSystemControl.h>
...
namespace SparkyStudios::Audio::Amplitude
{
    static TImplControlType TagToType(const AZStd::string_view tag)
    {
        ...
        if (tag == XmlTags::kRtpcTag)
            return eAMCT_AMPLITUDE_RTPC;

        if (tag == XmlTags::kFileTag)
            return eAMCT_AMPLITUDE_SOUND_BANK;

        if (tag == XmlTags::kSwitchTag)
            return eAMCT_AMPLITUDE_SWITCH;

        if (tag == XmlTags::kSwitchStateTag)
            return eAMCT_AMPLITUDE_SWITCH;

        if (tag == XmlTags::kEnvironmentTag)
            return eAMCT_AMPLITUDE_EFFECT;

        return eAMCT_INVALID;
    }

    static AZStd::string_view TypeToTag(const TImplControlType type)
    {
        switch (type)
        {
    ...
    void AmplitudeAudioSystemEditor::Reload()
    {
        ...
        for (const auto& idControlPair : _controls)
            if (const TControlPtr control = idControlPair.second)
    ...
    IAudioSystemControl* AmplitudeAudioSystemEditor::CreateControl(const SControlDef& controlDefinition)
    {
        ...
        if (IAudioSystemControl* control = GetControl(id))
        {
    ...
    IAudioSystemControl* AmplitudeAudioSystemEditor::GetControl(CID id) const
    {
        if (id != ACE_INVALID_CID)
            if (const auto it = _controls.find(id); it != _controls.end())
        ...
        AZStd::string name, bool isLocalized, IAudioSystemControl* parent) const
    {
        ...
        if (isLocalized)
            AZ::StringFunc::Path::Join(_loader.GetLocalizationFolder().c_str(), name.c_str(), name);
        ...
        EACEControlType atlControlType, IAudioSystemControl* middlewareControl)
    {
        if (middlewareControl)
        {
            ...
            ++_connectionsByID[middlewareControl->GetId()];
            ...
            if (middlewareControl->GetType() == eAMCT_AMPLITUDE_RTPC)
            {
                ...
                case EACEControlType::eACET_SWITCH_STATE:
                    {
                        return AZStd::make_shared<SwitchStateConnection>(middlewareControl->GetId());
                    }
                case EACEControlType::eACET_ENVIRONMENT:
                    {
                        return AZStd::make_shared<EffectConnection>(middlewareControl->GetId());
                    }
                }
            }
        ...
        AZ::rapidxml::xml_node<char>* node, EACEControlType atlControlType)
    {
        if (node)
        {
            const AZStd::string_view element(node->name());
            if (const TImplControlType type = TagToType(element); type != AUDIO_IMPL_INVALID_TYPE)
            {
                AZStd::string name;
                AZStd::string_view localized;

                if (const auto nameAttr = node->first_attribute(XmlTags::kNameAttribute, 0, false); nameAttr != nullptr)
                    name = nameAttr->value();

                if (const auto localizedAttr = node->first_attribute(XmlTags::kLocalizedAttribute, 0, false); localizedAttr != nullptr)
                    localized = localizedAttr->value();

                const bool isLocalized = AZ::StringFunc::Equal(localized, "true");
                ...
                {
                    if (const auto childNode = node->first_node(); childNode != nullptr)
                    {
                        AZStd::string childName;
                        if (const auto childNameAttr = childNode->first_attribute(XmlTags::kNameAttribute, 0, false);
                ...
                if (control)
                {
                    ...
                    ++_connectionsByID[control->GetId()];
                    ...
                    if (type == eAMCT_AMPLITUDE_RTPC)
                    {
                        ...
                        case EACEControlType::eACET_RTPC:
                            {
                                TRtpcConnectionPtr connection = AZStd::make_shared<RtpcConnection>(control->GetId());
                                ...
                                connection->mMultiplier = mult;
                                connection->mShift = shift;

                                return connection;
                            }
                        case EACEControlType::eACET_SWITCH_STATE:
                            {
                                TSwitchStateConnectionPtr connection = AZStd::make_shared<SwitchStateConnection>(control->GetId());

                                AmObjectID state = 0.0f;
                                if (const auto valueAttr = node->first_attribute(XmlTags::kValueAttribute, 0, false); valueAttr != nullptr)
    ...
    {
        if (const auto* control = dynamic_cast<AmplitudeAudioSystemControl*>(GetControl(connection->GetID())))
        {
            ...
            case AudioControls::eAMCT_AMPLITUDE_SWITCH:
                {
                    if (const auto* parent = dynamic_cast<AmplitudeAudioSystemControl*>(control->GetParent()))
                    {
                        const AZStd::string_view parentType = TypeToTag(parent->GetType());
                        const auto switchNode =
                            xmlAllocator.allocate_node(AZ::rapidxml::node_element, xmlAllocator.allocate_string(parentType.data()));

                        const auto switchNameAttr = xmlAllocator.allocate_attribute(
                            XmlTags::kNameAttribute, xmlAllocator.allocate_string(parent->GetName().c_str()));

                        const auto switchIdAttr = xmlAllocator.allocate_attribute(
                            XmlTags::kIdAttribute, xmlAllocator.allocate_string(AZStd::to_string(parent->GetAmplitudeId()).c_str()));

                        const auto stateNode = xmlAllocator.allocate_node(AZ::rapidxml::node_element, XmlTags::kSwitchStateTag);

                        const auto stateNameAttr = xmlAllocator.allocate_attribute(
                            XmlTags::kNameAttribute, xmlAllocator.allocate_string(control->GetName().c_str()));

                        const auto stateIdAttr = xmlAllocator.allocate_attribute(
            ...
            case AudioControls::eAMCT_AMPLITUDE_RTPC:
                {
                    const auto connectionNode = xmlAllocator.allocate_node(
                        AZ::rapidxml::node_element, xmlAllocator.allocate_string(TypeToTag(control->GetType()).data()));

                    const auto idAttr = xmlAllocator.allocate_attribute(
                        XmlTags::kIdAttribute, xmlAllocator.allocate_string(AZStd::to_string(control->GetAmplitudeId()).c_str()));
                    ...
                    {
                        const AZStd::shared_ptr<const RtpcConnection> rtpcConnection =
                            AZStd::static_pointer_cast<const RtpcConnection>(connection);

                        if (rtpcConnection->mMultiplier != 1.f)
                        {
                            auto multAttr = xmlAllocator.allocate_attribute(
                                XmlTags::kMultiplierAttribute,
                                xmlAllocator.allocate_string(AZStd::to_string(rtpcConnection->mMultiplier).c_str()));

                            connectionNode->append_attribute(multAttr);
                        }

                        if (rtpcConnection->mShift != 0.f)
                        {
                            auto shiftAttr = xmlAllocator.allocate_attribute(
                                XmlTags::kShiftAttribute, xmlAllocator.allocate_string(AZStd::to_string(rtpcConnection->mShift).c_str()));
                    ...
                    {
                        const AZStd::shared_ptr<const SwitchStateConnection> stateConnection =
                            AZStd::static_pointer_cast<const SwitchStateConnection>(connection);

                        const auto valueAttr = xmlAllocator.allocate_attribute(
            ...
            case AudioControls::eAMCT_AMPLITUDE_EFFECT:
                {
                    ...
                    const AZStd::shared_ptr<const EffectConnection> effectConnection =
                        AZStd::static_pointer_cast<const EffectConnection>(connection);

                    const auto idAttr = xmlAllocator.allocate_attribute(
                        XmlTags::kIdAttribute, xmlAllocator.allocate_string(AZStd::to_string(control->GetAmplitudeId()).c_str()));

                    const auto effectAttr = xmlAllocator.allocate_attribute(
                        XmlTags::kValueAttribute, xmlAllocator.allocate_string(AZStd::to_string(effectConnection->mEffect).c_str()));
            ...
            case AudioControls::eAMCT_AMPLITUDE_SOUND_BANK:
                {
                    ...
                    {
                        const auto locAttr =
                            xmlAllocator.allocate_attribute(XmlTags::kLocalizedAttribute, xmlAllocator.allocate_string("true"));
    ...
    {
        for (const auto& idCountPair : _connectionsByID)
            if (idCountPair.second > 0)
                if (IAudioSystemControl* control = GetControl(idCountPair.first))
                    control->SetConnected(true);
    }

    void AmplitudeAudioSystemEditor::ConnectionRemoved(IAudioSystemControl* control)
    {
        int connectionCount = _connectionsByID[control->GetId()] - 1;
        if (connectionCount <= 0)
        {
            connectionCount = 0;
            control->SetConnected(false);
        }
        _connectionsByID[control->GetId()] = connectionCount;
    }

    AZ::IO::FixedMaxPath AmplitudeAudioSystemEditor::GetDataPath() const
    {
        AZ::SettingsRegistryInterface::FixedValueString projectSourcePath;

        if (auto* const settingsRegistry = AZ::SettingsRegistry::Get(); settingsRegistry != nullptr)
            settingsRegistry->Get(projectSourcePath, kProjectSourcesPathSettingsRegistryKey);

        if (projectSourcePath.empty())
            projectSourcePath = kDefaultProjectSourcesPath;

        const auto projectPath = AZ::IO::FixedMaxPath{ AZ::Utils::GetProjectPath() };
        return projectPath / projectSourcePath;
    }
} // namespace SparkyStudios::Audio::Amplitude
